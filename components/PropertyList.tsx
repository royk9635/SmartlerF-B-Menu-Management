...
const PropertyList: React.FC<PropertyListProps> = ({ properties, onAddProperty, onEditProperty, onDeleteProperty, onSelectProperty }) => {
    return (
        <div className="bg-white shadow-md rounded-lg border border-slate-200 p-6">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-slate-700">Properties</h2>
                <button
                            ...
                            <tr key={property.id} className="border-b border-slate-200">
                                ...
                                <td className="py-3 px-4 text-center">
                                    <div className="flex items-center justify-center space-x-2">
                                        <button onClick={() => onSelectProperty(property)} className="text-sky-500 hover:text-sky-700 p-2 rounded-full hover:bg-sky-100 transition" title="View Restaurants">
                                            <EyeIcon className="h-5 w-5"/>
                                        </button>
                                        <button onClick={() => onEditProperty(property)} className="text-amber-500 hover:text-amber-700 p-2 rounded-full hover:bg-amber-100 transition" title="Edit Property">
                                            <PencilIcon className="h-5 w-5"/>
                                        </button>
                                        <button onClick={() => onDeleteProperty(property.id)} className="text-rose-500 hover:text-rose-700 p-2 rounded-full hover:bg-rose-100 transition" title="Delete Property">
